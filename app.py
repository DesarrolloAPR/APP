...
if img_file_buffer is not None:
    ...
    for r in results:
        ...
        # Lógica de detección de nombres de clase
        for box in r.boxes:
            cls_id = int(box.cls)
            label = model.names[cls_id].lower()
            
            # Verifica si la clase detectada es casco (según tu data.yaml)
            if "helmet" in label or "casco" in label:
                tiene_casco = True

        # Dibujar línea de zona
        if detectar_zona:
            # Línea roja horizontal a mitad de pantalla
            cv2.line(annotated_frame, (0, h//2), (w, h//2), (255, 0, 0), 5)

    # Mostrar resultados
    st.image(annotated_frame, channels="RGB", caption="Análisis en Tiempo Real")
    ...
    with col1:
        if tiene_casco:
            ...
            st.error("⚠️ Alerta: Sin Casco")
            
    with col2:
        # Aquí podrías añadir lógica si las coordenadas de 'box' superan h//2
        st.info("Zona monitoreada activa")
